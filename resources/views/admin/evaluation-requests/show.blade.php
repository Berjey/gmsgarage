...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.evaluation-requests.index') }}"
           class="inline-flex items-center px-3 py-2 bg-white hover:bg-gray-50 text-gray-700 rounded-lg font-medium transition-colors border border-gray-300 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('admin.evaluation-requests.read', $request->id) }}" method="POST">
                @csrf
                <button type="submit" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-semibold shadow-sm transition-colors">
            ...
            <form action="{{ route('admin.evaluation-requests.destroy', $request->id) }}" method="POST"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 border border-red-300 text-red-600 hover:bg-red-50 rounded-xl font-semibold transition-colors">
                    İsteği Sil
                </button>
            </form>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center gap-3">
            ...
            <div class="min-w-0">
                <p class="text-xs text-gray-400 truncate">Araç</p>
                <p class="text-sm font-bold text-gray-900 truncate">{{ $request->brand }} {{ $request->model }}</p>
                <p class="text-xs text-gray-500">{{ $request->year }}</p>
            </div>
        </div>
        <div class="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-400">Kilometre</p>
                <p class="text-base font-bold text-primary-600">{{ number_format($request->mileage, 0, ',', '.') }}</p>
                <p class="text-xs text-gray-500">km</p>
            </div>
        </div>
        <div class="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center gap-3">
            @php $tBadge = in_array($tramerVal,['VAR','AGIR_HASAR']) ? ['bg-red-50','text-red-600','w-9 h-9 bg-red-50','text-red-500'] : ['bg-green-50','text-green-700','w-9 h-9 bg-green-50','text-green-500']; @endphp
            <div class="w-9 h-9 {{ $tBadge[2] }} rounded-lg flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 {{ $tBadge[3] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
            </div>
            <div>
                <p class="text-xs text-gray-400">Tramer / Hasar</p>
                <p class="text-sm font-bold {{ $tBadge[1] }}">{{ $tramerLabels[$tramerVal] ?? '-' }}</p>
                @if(!empty($extra['tramer_tutari']))<p class="text-xs text-gray-500">{{ number_format($extra['tramer_tutari'],0,',','.') }} TL</p>@endif
            </div>
        </div>
        <div class="bg-white rounded-xl px-4 py-3 border border-gray-200 shadow-sm flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-400">Talep Tarihi</p>
                ...
                <div class="px-6 py-4">
                    <dl class="grid grid-cols-2 gap-x-6 gap-y-3">
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Marka / Model</dt>
                            <dd class="text-sm font-bold text-gray-900 mt-0.5">{{ $request->brand }} {{ $request->model }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Yıl</dt>
                            <dd class="text-sm text-gray-900 mt-0.5">{{ $request->year }}</dd>
                        </div>
                        @if($request->version)
                        <div class="col-span-2 flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Versiyon</dt>
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $request->version }}</dd>
                        </div>
                        @endif
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Yakıt / Vites</dt>
                            <dd class="text-sm text-gray-900 mt-0.5">{{ $request->fuel_type }} / {{ $request->transmission }}</dd>
                        </div>
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Kilometre</dt>
                            <dd class="text-sm font-bold text-primary-600 mt-0.5">{{ number_format($request->mileage, 0, ',', '.') }} KM</dd>
                        </div>
                        @if(!empty($extra['govde_tipi']))
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Gövde Tipi</dt>
                            <dd class="text-sm text-gray-900 mt-0.5">{{ $extra['govde_tipi'] }}</dd>
                        </div>
                        @endif
                        @if(!empty($extra['renk']))
                        <div class="flex flex-col">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Renk</dt>
                            <dd class="text-sm text-gray-900 mt-0.5">{{ $extra['renk'] }}</dd>
                        </div>
                        @endif
                    </dl>
                    @if(!empty($extra['not']))
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Kullanıcı Notu</p>
                        <p class="text-sm text-gray-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2.5">{{ $extra['not'] }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3 flex-wrap">
                        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="w-3 h-3 rounded-full bg-white border border-gray-300 inline-block"></span>Orijinal</span>
                        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="w-3 h-3 rounded-full bg-blue-400 inline-block"></span>Boyalı</span>
                        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="w-3 h-3 rounded-full bg-yellow-400 inline-block"></span>Lokal Boyalı</span>
                        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="w-3 h-3 rounded-full bg-red-500 inline-block"></span>Değişmiş</span>
                    </div>
                </div>
                <div class="p-6 flex flex-col md:flex-row gap-6 items-start">
                    ...
                    {{-- Parça Listesi --}}
                    <div class="flex-1 grid grid-cols-2 xl:grid-cols-3 gap-1.5">
                        @foreach($ekspertiz as $partKey => $partVal)
                        @php $c = $colorMap[$partVal] ?? ['bg'=>'bg-gray-100','text'=>'text-gray-700','fill'=>'#FFFFFF','label'=>$partVal]; @endphp
                        <div class="flex items-center justify-between bg-gray-50 hover:bg-white rounded-lg px-2.5 py-1.5 border border-gray-100 transition-colors">
                            <span class="text-xs text-gray-600 truncate mr-2">{{ $partLabels[$partKey] ?? $partKey }}</span>
                            <span class="text-xs font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 {{ $c['bg'] }} {{ $c['text'] }}">{{ $c['label'] }}</span>
                        </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 space-y-3">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Ad Soyad</p>
                        <p class="text-base font-bold text-gray-900 mt-0.5">{{ $request->name }}</p>
                    </div>
                    <div class="grid grid-cols-1 gap-3 pt-1">
                        <div class="flex items-center gap-2.5 bg-gray-50 rounded-lg px-3 py-2">
                ...
                <div class="px-6 pb-6 flex gap-2">
                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $request->phone) }}"
                       class="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-green-500/20">
                    ...
                    <a href="https://wa.me/90{{ preg_replace('/[^0-9]/', '', ltrim($request->phone, '0')) }}" target="_blank"
                       class="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold transition-all shadow-lg shadow-green-400/20">
