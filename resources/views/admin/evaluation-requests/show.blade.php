...
<div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.evaluation-requests.pdf', $request->id) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">
                ...
                <form action="{{ route('admin.evaluation-requests.destroy', $request->id) }}" method="POST" class="inline-block"
                    ...
                    @method('DELETE')
                    <button type="submit"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-colors text-sm font-medium">
    ...
    <div class="space-y-6">
        ...
        {{-- ÜST BLOK: Araç + Müşteri Bilgileri Yan Yana --}}
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden h-full">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-4 border-t border-gray-50">
                        ...
                        @if($request->email)
                        <button type="button"
                                onclick="openSendEmailModal('eval-email-modal')"
                                class="col-span-2 flex items-center justify-center gap-2 px-4 py-3 bg-white text-gray-700 font-bold rounded-xl border border-gray-200 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all">
        ...
        @if(!empty($ekspertiz))
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    {{-- Sol Kolon: SVG + Legend --}}
                    <div class="flex flex-col items-center">
                        <div class="w-64 bg-gray-50 rounded-xl border border-gray-100 p-3">
                        ...
                        <div class="flex justify-center items-center gap-4 flex-wrap mt-4">
                            <div class="flex items-center gap-1.5 text-xs font-medium text-gray-500"><span class="w-3 h-3 rounded-sm bg-white border border-gray-300 flex-shrink-0"></span>Orijinal</div>
                            <div class="flex items-center gap-1.5 text-xs font-medium text-gray-500"><span class="w-3 h-3 rounded-sm bg-blue-400 flex-shrink-0"></span>Boyalı</div>
                            <div class="flex items-center gap-1.5 text-xs font-medium text-gray-500"><span class="w-3 h-3 rounded-sm bg-yellow-400 flex-shrink-0"></span>Lokal Boyalı</div>
                            <div class="flex items-center gap-1.5 text-xs font-medium text-gray-500"><span class="w-3 h-3 rounded-sm bg-red-500 flex-shrink-0"></span>Değişmiş</div>
                        </div>
                    </div>{{-- /Sol Kolon --}}

                    {{-- Sağ Kolon: Parça Listesi --}}
                    <div class="grid grid-cols-2 gap-2 self-start">
                        ...
                        <div class="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-lg px-3 py-2 gap-2">
                            <span class="text-xs text-gray-700 font-medium truncate">{{ $partLabels[$partKey] ?? $partKey }}</span>
...
@if($request->email)
    <x-admin.send-email-modal
        modalId="eval-email-modal"
        postUrl="{{ route('admin.evaluation-requests.send-email', $request->id) }}"
