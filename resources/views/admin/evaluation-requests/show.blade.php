...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.evaluation-requests.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-gray-50 text-gray-600 hover:text-gray-900 rounded-xl font-medium transition-colors border border-gray-200 shadow-sm text-sm">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('admin.evaluation-requests.read', $request->id) }}" method="POST">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-semibold text-sm shadow-sm transition-colors">
            ...
            <form action="{{ route('admin.evaluation-requests.destroy', $request->id) }}" method="POST"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 border border-red-200 text-red-600 hover:bg-red-50 rounded-xl font-semibold text-sm transition-colors">
    ...
    {{-- Ana İçerik: 12 kolon grid --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">

        {{-- SOL KOLON (col-span-7): Araç Bilgileri + İletişim Bilgileri --}}
        ...
        <div>
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden h-full">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                    ...
                    <h2 class="text-base font-bold text-gray-900">Araç Bilgileri</h2>
                </div>
                <div class="p-6">
                    <dl class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-5">
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Marka / Model</dt>
                            <dd class="text-sm font-bold text-gray-900">{{ $request->brand }} {{ $request->model }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Yıl</dt>
                            <dd class="text-sm font-semibold text-gray-800">{{ $request->year }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Kilometre</dt>
                            <dd class="text-sm font-bold text-primary-600">{{ number_format($request->mileage, 0, ',', '.') }} KM</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Yakıt / Vites</dt>
                            <dd class="text-sm font-semibold text-gray-800">{{ $request->fuel_type }} / {{ $request->transmission }}</dd>
                        </div>
                        @if(!empty($extra['govde_tipi']))
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Gövde Tipi</dt>
                            <dd class="text-sm font-semibold text-gray-800">{{ $extra['govde_tipi'] }}</dd>
                        </div>
                        @endif
                        @if(!empty($extra['renk']))
                        <div>
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Renk</dt>
                            <dd class="text-sm font-semibold text-gray-800">{{ $extra['renk'] }}</dd>
                        </div>
                        @endif
                        @if($request->version)
                        <div class="col-span-2 sm:col-span-3">
                            <dt class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Versiyon</dt>
                            <dd class="text-sm font-semibold text-gray-800">{{ $request->version }}</dd>
                        </div>
                        @endif
                    </dl>
                    @if(!empty($extra['not']))
                    <div class="mt-5 border-l-4 border-primary-400 bg-primary-50/60 rounded-r-lg px-4 py-3">
                        <p class="text-xs font-bold text-primary-600 uppercase tracking-wider mb-1">Kullanıcı Notu</p>
                        <p class="text-sm text-gray-700">{{ $extra['not'] }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                ...
                <h2 class="text-base font-bold text-gray-900">İletişim Bilgileri</h2>
            </div>
            <div class="p-6">
                <div class="mb-4">
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Ad Soyad</p>
                    <p class="text-lg font-bold text-gray-900">{{ $request->name }}</p>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-5">
                    <div class="flex items-center gap-3 py-2.5 px-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $request->phone }}</span>
                    </div>
                    <div class="flex items-center gap-3 py-2.5 px-3 bg-gray-50 rounded-lg">
                        <svg class="w-4 h-4 text-primary-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        <span class="text-sm text-gray-700 truncate">{{ $request->email ?? 'Belirtilmemiş' }}</span>
                    </div>
                    <div class="flex items-center gap-3 py-2.5 px-3 bg-gray-50 rounded-lg">
                        <svg class="w-4 h-4 text-primary-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        <span class="text-sm text-gray-700">{{ $request->created_at->format('d.m.Y H:i') }}</span>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-2">
                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $request->phone) }}"
                       class="flex items-center justify-center gap-2 px-3 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-semibold text-sm transition-colors shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        Ara
                    </a>
                    <a href="https://wa.me/90{{ preg_replace('/[^0-9]/', '', ltrim($request->phone, '0')) }}" target="_blank"
                       class="flex items-center justify-center gap-2 px-3 py-2.5 text-white rounded-xl font-semibold text-sm transition-colors shadow-sm" style="background:#25D366;">
        ...
        {{-- SAĞ KOLON (col-span-5): Ekspertiz Durumu --}}
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            ...
            <h2 class="text-base font-bold text-gray-900">Ekspertiz Durumu</h2>
        </div>
        <div class="p-6 flex flex-col lg:flex-row gap-8 items-start">

            {{-- Araç SVG — sabit genişlik, ortalanmış --}}
            <div class="flex flex-col items-center gap-4 flex-shrink-0 w-full lg:w-52">
                <div class="w-full max-w-[180px] mx-auto bg-gray-50 rounded-xl p-3 border border-gray-100">
                ...
                {{-- Legend --}}
                <div class="flex flex-wrap justify-center gap-x-3 gap-y-1.5 pt-1">
                    <span class="flex items-center gap-1.5 text-xs text-gray-500 font-medium"><span class="w-3 h-3 rounded-full bg-white border-2 border-gray-300 flex-shrink-0"></span>Orijinal</span>
                    <span class="flex items-center gap-1.5 text-xs text-gray-500 font-medium"><span class="w-3 h-3 rounded-full bg-blue-400 flex-shrink-0"></span>Boyalı</span>
                    <span class="flex items-center gap-1.5 text-xs text-gray-500 font-medium"><span class="w-3 h-3 rounded-full bg-yellow-400 flex-shrink-0"></span>Lokal Boyalı</span>
                    <span class="flex items-center gap-1.5 text-xs text-gray-500 font-medium"><span class="w-3 h-3 rounded-full bg-red-500 flex-shrink-0"></span>Değişmiş</span>
                </div>
            </div>

            {{-- Parça Listesi --}}
            <div class="flex-1 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2">
                ...
                <div class="flex items-center justify-between rounded-lg px-3 py-2.5 border border-gray-100 bg-gray-50 hover:bg-white transition-colors">
                    <span class="text-xs font-medium text-gray-600 truncate mr-2">{{ $partLabels[$partKey] ?? $partKey }}</span>
                    <span class="text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 {{ $c['bg'] }} {{ $c['text'] }}">{{ $c['label'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
