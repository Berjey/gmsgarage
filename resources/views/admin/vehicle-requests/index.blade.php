...
<div class="bg-white dark:bg-[#252525] rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
    <div class="p-6 border-b border-gray-200 dark:border-gray-800">
        <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Araç İstekleri</h2>
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Gelen araç isteklerini görüntüleyin</p>
    </div>
            ...
            <tbody class="bg-white dark:bg-[#252525] divide-y divide-gray-200 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-[#2a2a2a] transition-colors {{ !$request->is_read ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <a href="{{ route('admin.vehicle-requests.show', $request->id) }}" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300">Görüntüle</a>
                        <form action="{{ route('admin.vehicle-requests.destroy', $request->id) }}" method="POST" class="inline" onsubmit="return confirm('Bu isteği silmek istediğinize emin misiniz?');">
                            @csrf
                            @method('DELETE')
