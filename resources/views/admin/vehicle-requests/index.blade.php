...
@section('content')
@php
    $totalReqs  = \App\Models\VehicleRequest::count();
    $newReqs    = \App\Models\VehicleRequest::where('is_read', false)->count();
    $readReqs   = \App\Models\VehicleRequest::where('is_read', true)->count();
@endphp
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-xl p-6 border-2 border-primary-100 shadow-sm group">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-600 text-sm font-medium mb-1">Toplam İstek</p>
    ...
    <div class="bg-white rounded-xl p-6 border-2 border-gray-100 shadow-sm hover:shadow-lg hover:border-blue-300 transition-all group">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{{ $newReqs }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 border-2 border-gray-100 shadow-sm hover:shadow-lg hover:border-green-300 transition-all group">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 group-hover:text-green-600 transition-colors">{{ $readReqs }}</p>
            </div>
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-2">Toplam <span class="font-bold text-primary-600">{{ $totalReqs }}</span> istek • <span class="font-bold text-blue-600">{{ $newReqs }}</span> yeni</p>
            </div>
        </div>
    </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50/50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">İstenen Araç</th>
