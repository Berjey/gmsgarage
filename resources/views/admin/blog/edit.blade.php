...
@section('title', 'Blog Yazısını Düzenle - Admin Panel')
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Blog Yazısını Düzenle</h1>
    ...
    <form action="{{ route('admin.blog.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mt-2 text-sm text-gray-500">
                            ...
                            <span class="text-primary-600">{{ config('app.url') }}/blog/{{ $post->slug }}</span>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="excerpt" 
                              ...
                              class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all resize-none">{{ old('excerpt', $post->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" 
                              ...
                              class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-mono text-sm resize-none">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Dosya Yükleme -->
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Yeni Dosya Yükle</label>
                        <div class="relative">
                            <input type="file" 
                                   name="featured_image" 
                                   id="image-upload"
                ...
                <div id="image-preview" class="hidden mt-6">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Yeni Görsel Önizleme</label>
                    <div class="relative inline-block">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="meta_description" 
                                  ...
                                  class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all resize-none">{{ old('meta_description', $post->meta_description) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('meta_keywords_string', is_array($post->meta_keywords) ? implode(', ', $post->meta_keywords) : '') }}"
        ...
        <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl border border-purple-100 shadow-sm overflow-hidden">
            <div class="p-6">
                <h3 class="text-lg font-bold text-gray-900 mb-4">İstatistikler</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="bg-white rounded-xl p-4 border border-purple-100">
                        <div class="text-sm text-gray-500 mb-1">Görüntülenme</div>
                        <div class="text-2xl font-bold text-purple-600">{{ number_format($post->views) }}</div>
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-indigo-100">
                        <div class="text-sm text-gray-500 mb-1">Okuma Süresi</div>
                        <div class="text-2xl font-bold text-indigo-600">{{ $post->reading_time ?? 1 }} dk</div>
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-blue-100">
                        <div class="text-sm text-gray-500 mb-1">Oluşturulma</div>
                        <div class="text-sm font-bold text-blue-600">{{ $post->created_at->format('d.m.Y H:i') }}</div>
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-green-100">
                        <div class="text-sm text-gray-500 mb-1">Son Güncelleme</div>
                        <div class="text-sm font-bold text-green-600">{{ $post->updated_at->format('d.m.Y H:i') }}</div>
                    </div>
                </div>
            </div>
        </div>
