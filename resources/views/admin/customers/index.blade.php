...
<div class="container mx-auto">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Müşteri Listesi (CRM)</h1>
            <p class="text-gray-600 mt-1">Web sitesinden form dolduran kullanıcılar</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <!-- Export Button -->
            <a href="{{ route('admin.customers.export', request()->all()) }}" 
               class="px-5 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2 font-semibold">
            ...
            <button type="button" 
                    ...
                    class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 font-semibold">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <form id="customers-filter-form" method="GET" action="{{ route('admin.customers.index') }}" class="flex flex-wrap gap-4">
            ...
            <a href="{{ route('admin.customers.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                Temizle
            </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ 'bg-' . $customer->source_badge_color . '-100 text-' . $customer->source_badge_color . '-800' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($customer->kvkk_consent)
                                <span class="text-green-600 flex items-center gap-1">
                                    ...
                                    <span class="text-xs">Onaylı</span>
                                </span>
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                            ...
                            <form action="{{ route('admin.customers.destroy', $customer->id) }}" method="POST" class="inline-block" onsubmit="return confirmDelete(this, '{{ $customer->name }} müşterisini')">
                                @csrf
                                @method('DELETE')
        ...
        <div class="p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Henüz müşteri yok</h3>
            <p class="text-gray-600">Web sitesinden form dolduran kullanıcılar burada görünecek.</p>
        </div>
        @endif
    </div>

    <!-- Stats Card -->
    <div class="mt-6 bg-gradient-to-r from-primary-600 to-primary-700 rounded-lg shadow-lg p-6 text-white">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="text-center">
                <div class="text-3xl font-bold">{{ $customers->total() }}</div>
                <div class="text-sm text-primary-100">Toplam Müşteri</div>
            </div>
            <div class="text-center">
                <div class="text-3xl font-bold">{{ \App\Models\Customer::where('kvkk_consent', true)->count() }}</div>
                <div class="text-sm text-primary-100">KVKK Onaylı</div>
            </div>
            <div class="text-center">
                <div class="text-3xl font-bold">{{ \App\Models\Customer::whereDate('created_at', today())->count() }}</div>
                <div class="text-sm text-primary-100">Bugün Eklenen</div>
            </div>
        </div>
    </div>
...
<div id="bulk-email-modal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-[9999] opacity-0 invisible transition-all duration-300">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full m-4 relative transform scale-95 opacity-0 transition-all duration-300">
        <!-- Close Button -->
        <button type="button" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors z-10" onclick="closeBulkEmailModal()">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>

        <div class="p-6">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">Toplu E-posta Gönder</h3>
            <p class="text-gray-600 dark:text-gray-300 mb-6">Seçili müşterilere toplu e-posta gönderin.</p>
            ...
            <form action="{{ route('admin.customers.send-bulk-email') }}" method="POST" id="bulk-email-form">
                ...
                <div class="space-y-4">
                    <!-- Subject -->
                    <div>
                        <label for="email-subject" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">E-posta Konusu</label>
                        <input type="text" name="subject" id="email-subject" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" placeholder="Örn: Özel Kampanya Duyurusu">
                    </div>

                    <!-- Message -->
                    <div>
                        <label for="email-message" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Mesaj</label>
                        <textarea name="message" id="email-message" rows="8" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" placeholder="E-posta içeriğinizi buraya yazın..."></textarea>
                        <p class="text-xs text-gray-500 mt-1">Not: HTML etiketleri desteklenmez, düz metin olarak gönderilir.</p>
                    </div>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" onclick="closeBulkEmailModal()" class="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors font-semibold">
                        İptal
                    </button>
...
document.getElementById('bulk-email-modal')?.addEventListener('click', (e) => {
    if (e.target.id === 'bulk-email-modal') {
        closeBulkEmailModal();
    }
});
</script>
