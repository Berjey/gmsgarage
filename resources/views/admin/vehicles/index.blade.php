...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <a href="{{ route('admin.vehicles.index') }}" class="bg-white rounded-xl p-5 border-2 border-primary-100 shadow-sm hover:border-primary-300 transition-all cursor-pointer group">
    ...
    <a href="{{ route('admin.vehicles.index', ['status' => 'active']) }}" class="bg-white rounded-xl p-5 border-2 border-gray-100 shadow-sm hover:border-green-300 transition-all cursor-pointer group">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Aktif İlanlar</p>
    ...
    <a href="{{ route('admin.vehicles.index', ['status' => 'featured']) }}" class="bg-white rounded-xl p-5 border-2 border-gray-100 shadow-sm hover:border-yellow-300 transition-all cursor-pointer group">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Öne Çıkan</p>
                <p class="text-3xl font-bold text-gray-900 group-hover:text-yellow-600 transition-colors">{{ $featuredCount }}</p>
            </div>
            <div class="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
    ...
    <a href="{{ route('admin.vehicles.index', ['status' => 'passive']) }}" class="bg-white rounded-xl p-5 border-2 border-gray-100 shadow-sm hover:border-gray-300 transition-all cursor-pointer group">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Pasif İlanlar</p>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('vehicles.show', $vehicle->slug ?: $vehicle->id) }}" 
                                   target="_blank"
                                   class="p-1.5 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-600 hover:text-white transition-all border border-blue-200 shadow-sm" 
                                ...
                                <a href="{{ route('admin.vehicles.edit', $vehicle->id) }}" 
                                   class="p-1.5 text-amber-600 bg-amber-50 rounded-lg hover:bg-amber-600 hover:text-white transition-all border border-amber-200 shadow-sm" 
                                   title="Düzenle">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('admin.vehicles.destroy', $vehicle->id) }}" 
                                      method="POST" 
                                      class="inline-block"
                                      onsubmit="return confirmDelete(this, '{{ addslashes($vehicle->brand . ' ' . $vehicle->model) }} aracını')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="p-1.5 text-red-600 bg-red-50 rounded-lg hover:bg-red-600 hover:text-white transition-all border border-red-200 shadow-sm" 
                                            title="Sil">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
const vehicleRoutes = {
    toggleActive: @json(url('/admin/vehicles')),
};
...
function toggleVehicleStatus(vehicleId, type, value, event) {
    ...
    const endpoint = type === 'active' ? 'toggle-active' : 'toggle-featured';
    const url = `${vehicleRoutes.toggleActive}/${vehicleId}/${endpoint}`;
    const statusText = type === 'active'
        ? (value ? 'Araç yayına alındı' : 'Araç yayından kaldırıldı')
        : (value ? 'Araç öne çıkarıldı' : 'Araç öne çıkandan kaldırıldı');
...
function updateLabels(checkbox, type, value) {
    ...
    if (type === 'active') {
        label.className = value
            ? 'text-xs font-bold px-2 py-1 rounded text-green-700 bg-green-50'
            : 'text-xs font-bold px-2 py-1 rounded text-gray-600 bg-gray-100';
        label.textContent = value ? 'Yayında' : 'Yayında Değil';
    } else if (type === 'featured') {
        label.className = value
            ? 'text-xs font-bold px-2 py-1 rounded text-yellow-700 bg-yellow-50'
            : 'text-xs font-bold px-2 py-1 rounded text-gray-600 bg-gray-100';
        if (value) {
...
function showNotification(message, type) {
    ...
    n.className = `fixed top-4 right-4 px-6 py-4 rounded-xl shadow-2xl text-white font-semibold z-50 transition-all transform flex items-center gap-3 ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`;
    const icon = type === 'success'
        ? '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>'
        : '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>';
    n.innerHTML = icon + '<span>' + message + '</span>';
