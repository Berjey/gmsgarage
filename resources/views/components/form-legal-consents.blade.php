...
@if($formPages->count() > 0)
    <div class="space-y-3">
        ...
        <div class="bg-gray-50 dark:bg-neutral-900 border-2 border-gray-200 dark:border-neutral-800 rounded-lg p-4 transition-colors">
            <label class="flex items-start space-x-3 cursor-pointer group">
                <input type="checkbox" 
                       name="legal_consent_{{ $page->slug }}" 
                       id="legal-{{ $page->slug }}-{{ $formId ?? 'default' }}" 
                       required
                       class="mt-1 w-5 h-5 text-red-600 border-gray-300 dark:border-neutral-600 rounded focus:ring-2 focus:ring-red-500 dark:bg-neutral-950 dark:focus:ring-red-600">
                <span class="flex-1 text-sm text-gray-700 dark:text-neutral-300">
                    ...
                    metnini okudum, kabul ediyorum. <span class="text-red-500">*</span>
                </span>
            </label>
